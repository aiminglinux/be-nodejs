pipeline {
    ...
    stages {
        ...
        stage('Checkout argocd repo') {
            steps {
                withCredentials([file(credentialsId: 'gitlab-creds', variable: 'credsFile')]) {
                    git url: 'git@gitlab.com:aiming.fb/freeman-argocd.git', branch: 'main', credentialsId: 'credsFile'
                }
            }
        }
